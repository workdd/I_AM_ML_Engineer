...
원문 그림을 보지 않고도 세 가지 구조만 잡으면 나머지가 따라옵니다.

### CED: prefill 절반만 계산한다

```mermaid
flowchart TB
    subgraph ENC["Causal Encoder: 레이어 1 에서 20"]
        E["각 레이어가 자기 은닉 상태에서<br/>글로벌 KV 를 직접 생성한다"]
    end
    H["레이어 20 의 은닉 상태<br/>H(L/2)"]
    subgraph DEC["Decoder: 레이어 21 에서 40"]
        D["글로벌 KV 를 자기 은닉 상태가 아니라<br/>H(L/2) 에서 레이어별 가중치로 투영한다"]
    end
    ENC --> H --> DEC

    P1["prefill: 하위 20층만 계산하면<br/>상위 20층 글로벌 KV 가 함께 얻어진다"]
    P2["decode: 40층 전부 활성<br/>활성 파라미터 16B"]
    DEC -.-> P1
    DEC -.-> P2

    classDef enc fill:#eef3ee,stroke:#3d6b45,color:#1e1c1a
    classDef dec fill:#f4f2ee,stroke:#8a837a,color:#1e1c1a
    classDef note fill:#fff,stroke:#e2ded7,color:#5b554e
    class E enc
    class D dec
    class P1,P2,H note
```

핵심은 **prefill 활성 파라미터가 8B, decode 가 16B** 라는 비대칭입니다. 에이전트 워크로드는 툴 호출로 prefill 이 잦으므로 그 구간의 단가가 절반이 됩니다.

SWA 는 이 공유에서 빠집니다. 모든 레이어가 자기 은닉 상태에서 지역 KV 를 만들고, 그 비용은 Decoder SWA Bounded Replay 로 마지막 `n_win` 토큰만 prefill 해 줄입니다.

### CSA2: 레이어마다 무엇을 재사용하는가

```mermaid
flowchart LR
    subgraph FULL["Full Mode"]
        F1["글로벌 KV 생성"]
        F2["indexer 로 Top-K 선택"]
    end
    subgraph REIDX["Reindex Mode"]
        R1["앞 레이어 글로벌 KV 재사용"]
        R2["자기 indexer Q 로<br/>Top-K 를 다시 선택"]
    end
    subgraph REUSE["Reuse Mode"]
        U1["앞 레이어 글로벌 KV 재사용"]
        U2["앞 레이어 Top-K 인덱스도 재사용"]
    end
    FULL -->|"글로벌 KV 와 indexer K 를 넘긴다"| REIDX
    REIDX -->|"글로벌 KV 와 인덱스를 넘긴다"| REUSE

    OWN["세 모드 모두 자기 글로벌 Q 와<br/>자기 SWA KV 는 그대로 유지한다"]
    REUSE -.-> OWN

    classDef full fill:#eef3ee,stroke:#3d6b45,color:#1e1c1a
    classDef mid fill:#f7f4ec,stroke:#b8a86a,color:#1e1c1a
    classDef reuse fill:#f4f2ee,stroke:#8a837a,color:#1e1c1a
    classDef note fill:#fff,stroke:#e2ded7,color:#5b554e
    class F1,F2 full
    class R1,R2 mid
    class U1,U2 reuse
...
아래로 갈수록 재사용이 늘고 저장이 줄어듭니다. 실제 배치는 encoder 가 `Full 1 + Reuse 5` 를 3그룹, decoder 가 첫 그룹만 `Full 1 + Reuse 3` 이고 나머지 4그룹은 `Reindex 1 + Reuse 3` 입니다.

### 압축이 세 층위의 곱이다

```mermaid
flowchart TB
    A["아키텍처<br/>CSA2 레이어 간 글로벌 KV 와 인덱스 재사용"]
    B["캐시 정밀도<br/>학습 단계부터 FP4 글로벌 KV"]
    C["배포 전략<br/>SWA Bounded Replay 로 SWA KV 를 영속화하지 않는다"]

    G["글로벌 KV: 토큰당 890 바이트<br/>V4-Flash 의 약 1/4<br/>항상 HBM 에 상주"]
    P["영속 KV: V4-Flash 의 약 1/8<br/>SSD 또는 호스트 메모리"]

    A --> G
    B --> G
    C --> P
    G -.->|"글로벌 KV 가 1/4 로 줄어든 몫이<br/>영속 캐시에도 함께 반영된다"| P

    classDef layer fill:#eef3ee,stroke:#3d6b45,color:#1e1c1a
    classDef result fill:#fff,stroke:#8a837a,color:#1e1c1a
    class A,B,C layer
    class G,P result
```

**어느 하나만 떼어 적용하면 이 수치가 안 나옵니다.** 영속 KV 가 1/8 이 된 것도 두 곱셈 인자의 결과입니다. SWA KV 를 빼서 거의 절반, 남은 글로벌 KV 가 다시 1/4 입니다.
